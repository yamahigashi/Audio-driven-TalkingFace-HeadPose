...
from reconstruct_mesh import (
    Reconstruction_for_render_new,
    Reconstruction_for_render_new_given,
...
class RenderObject(object):
    def __init__(self, sess):
        ...
        self.faceshaper = tf.placeholder(
            name="face_shape_r", shape=[1, 35709, 3], dtype=tf.float32
        )
        self.facenormr = tf.placeholder(
            name="face_norm_r", shape=[1, 35709, 3], dtype=tf.float32
        )
        self.facecolor = tf.placeholder(
            name="face_color", shape=[1, 35709, 3], dtype=tf.float32
        )
        self.rendered = Render_layer(
            self.faceshaper, self.facenormr, self.facecolor, self.facemodel, 1
        )
        self.rendered2 = Render_layer2(
            self.faceshaper, self.facenormr, self.facecolor, self.facemodel, 1
        )
    ...
    def save_image(self, final_images, result_output_path):
        ...
        result_bytes = self.sess.run(
            self.encode_png, {self.rstimg: result_image * 255.0}
        )
    ...
    def show_clip_vertices(
        self, coef_path, clip_vertices, image_width=224, image_height=224
    ):
        ...
        for i in range(clip_vertices.shape[1]):
            if clip_vertices.shape[2] == 4:
                ...
                py = int(
                    image_height - 1 - round((v0y / v0w + 1.0) * half_image_height)
                )
    ...
    def gettexture(self, coef_path):
        ...
        (
            ...
            tri,
            face_projection,
        ) = Reconstruction_for_render_new(coef, self.facemodel, img_path)
    ...
    def render224_new(self, coef_path, result_output_path, tex2_path):
        ...
        (
            ...
            face_color2,
            tri,
        ) = Reconstruction_for_render_new_given(coef, self.facemodel, tex2_path)
        final_images = self.sess.run(
            self.rendered,
            feed_dict={
                self.faceshaper: face_shape_r.astype("float32"),
                self.facenormr: face_norm_r.astype("float32"),
                self.facecolor: face_color2.astype("float32"),
            },
        )
    ...
    def render224_new2(self, coef_path, result_output_path, tex2):
        ...
        (
            ...
            face_color2,
            tri,
        ) = Reconstruction_for_render_new_given2(coef, self.facemodel, tex2)
        final_images = self.sess.run(
            self.rendered,
            feed_dict={
                self.faceshaper: face_shape_r.astype("float32"),
    ...
    def render224(self, coef_path, result_output_path):
        ...
        final_images = self.sess.run(
            ...
            feed_dict={
                self.faceshaper: face_shape_r.astype("float32"),
                self.facenormr: face_norm_r.astype("float32"),
                self.facecolor: face_color.astype("float32"),
            },
        )
        # t01 = time.time()
        self.save_image(final_images, result_output_path)
...
if __name__ == "__main__":
    with tf.Session() as sess:
        ...
        result_output_path = (
            coef_path.replace("output/coeff", "output/render")[:-4] + "_rendernew.png"
        )
