...
def Shape_formation(id_coeff, ex_coeff, facemodel):
    face_shape = (
        np.einsum("ij,aj->ai", facemodel.idBase, id_coeff)
    ...
    face_shape = face_shape - np.mean(
        np.reshape(facemodel.meanshape, [1, -1, 3]), axis=1, keepdims=True
    )
...
def Compute_norm(face_shape, facemodel):
    ...
    face_norm = np.concatenate(
        [face_norm, np.zeros([1, 1, 3])], axis=1
    )  # concat face_normal with a zero vector at the end
    v_norm = np.sum(
        face_norm[:, point_id, :], axis=2
    )  # compute vertex normal using one-ring neighborhood
    v_norm = v_norm / np.expand_dims(
        np.linalg.norm(v_norm, axis=2), 2
...
def Texture_formation(tex_coeff, facemodel):
    ...
    face_texture = (
        np.einsum("ij,aj->ai", facemodel.texBase, tex_coeff) + facemodel.meantex
    )
...
def Projection_layer(
    face_shape, rotation, translation, focal=1015.0, center=112.0
    ...
    face_projection = aug_projection[:, :, 0:2] / np.reshape(
        aug_projection[:, :, 2], [1, np.shape(aug_projection)[1], 1]
    )
...
def Illumination_layer(face_texture, norm, gamma):
    ...
    Y6 = np.reshape(
        a2 * c2 * 0.5 / np.sqrt(3.0) * (3 * np.square(norm[:, :, 2]) - 1),
        [1, num_vertex, 1],
    )
    Y7 = np.reshape(-a2 * c2 * norm[:, :, 0] * norm[:, :, 2], [1, num_vertex, 1])
    Y8 = np.reshape(
        a2 * c2 * 0.5 * (np.square(norm[:, :, 0]) - np.square(norm[:, :, 1])),
        [1, num_vertex, 1],
    )
    ...
    lit_r = np.squeeze(
        np.matmul(Y, np.expand_dims(gamma[:, 0, :], 2)), 2
    ...
    face_color = np.stack(
        [
            lit_r * face_texture[:, :, 0],
            lit_g * face_texture[:, :, 1],
            lit_b * face_texture[:, :, 2],
        ],
        axis=2,
    )
...
def Illumination_inv_layer(face_color, lighting):
    face_texture = (
        np.stack(
            [
                face_color[:, :, 0] / lighting[:, :, 0],
                face_color[:, :, 1] / lighting[:, :, 1],
                face_color[:, :, 2] / lighting[:, :, 2],
            ],
            axis=2,
        )
        * 128
...
def Reconstruction(coeff, facemodel):
    ...
    face_projection = np.stack(
        [face_projection[:, :, 0], 224 - face_projection[:, :, 1]], axis=2
    )
...
def Project_layer(face_shape):
    ...
    # pdb.set_trace()
    clip_space_vertices = mesh_renderer.clip_vertices(
        face_shape,
