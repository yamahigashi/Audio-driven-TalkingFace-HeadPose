...
def demo_19news(n1, n2):
    ...
    for n in range(n1, n2):
        ...
        scale = 0.5 * (lm[0][0] - lm[1][0]) / (lm_new[0][0] - lm_new[1][0]) + 0.5 * (
            lm[3][0] - lm[4][0]
        ) / (lm_new[3][0] - lm_new[4][0])
        # print(scale)
        ...
        img.paste(
            img1, (trans[0], trans[1], trans[0] + img1.size[0], trans[1] + img1.size[1])
        )
        np.save(
            os.path.join("../Data", str(n), "transbig.npy"),
