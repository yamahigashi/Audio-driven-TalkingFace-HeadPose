...
def demo(image_path):
    ...
    already = [
        e[len(save_path) + 1 : -4].replace(name, image_path) + ".png" for e in already
    ]
    ...
    with tf.Graph().as_default() as graph:
        ...
        images = tf.placeholder(
            name="input_imgs", shape=[None, 224, 224, 3], dtype=tf.float32
        )
        ...
        with tf.Session() as sess:
            ...
            for file in img_list:
                ...
                if "_mtcnn" not in image_path:
                    ...
                    img, lm = load_img(
                        file, file[:-4].replace(name, name + "_mtcnn") + ".txt"
                    )
                ...
                cropped_output_path = os.path.join(
                    save_path2, file[:-4] + "_input2.png"
                )
                im.save(cropped_output_path)

                # save output files
                savemat(
                    os.path.join(save_path, file[:-4] + ".mat"),
                    {"coeff": coef, "lm_5p": lm_new2 - lm_new},
                )
    t2 = time.time()
