...
for n in range(26, 27):
    ...
    parser.add_argument(
        "--model", default="models/model-r100-ii/model,0", help="path to load model."
    )
    parser.add_argument(
        "--ga-model", default="models/gamodel-r50/model,0", help="path to load model."
    )
    ...
    parser.add_argument(
        "--imglist", default="trainB/" + str(n) + "_bmold_win3.txt", help="imglist name"
    )
    ...
    if not os.path.isdir(os.path.join(args.listdir, args.imglist)):
        imglist = (
            open(os.path.join(args.listdir, args.imglist), "r").read().splitlines()
        )
    else:
