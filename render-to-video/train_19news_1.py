...
def get_news(n):
    ...
    if "bmold" not in name:
        cmd = (
            "cd "
            + rootdir
            + "/..; matlab -nojvm -nosplash -nodesktop -nodisplay -r \"alpha_blend_news('"
            + video
            + "',"
            + str(start)
            + ","
        ...
        cmd = (
            "cd "
            + rootdir
            + "/..; matlab -nojvm -nosplash -nodesktop -nodisplay -r \"alpha_blend_newsold('"
            + video
            + "',"
            + str(start)
            + ","
...
cmd = (
    "cd arcface/; python test_batch.py --imglist trainB/%d_bmold_win3.txt --gpu %d"
    % (n, gpu_id)
)
os.system(cmd)
cmd = "cd arcface/; python test_batch.py --imglist testB/%d_bmold_win3.txt --gpu %d" % (
    n,
    gpu_id,
)
os.system(cmd)
...
n = str(n)
cmd = (
    "python train.py --dataroot %s_bmold_win3 --name memory_seq_p2p/%s --model memory_seq --continue_train --epoch 0 --epoch_count 1 --lambda_mask 2 --lr 0.0001 --display_env memory_seq_%s --gpu_ids %d --niter 60 --niter_decay 0"
    % (n, n, n, gpu_id)
)
os.system(cmd)
...
epoch = 60
cmd = (
    "python test.py --dataroot %s_bmold_win3 --name memory_seq_p2p/%s --model memory_seq --num_test 200 --epoch %d --gpu_ids %d --imagefolder images%d"
    % (n, n, epoch, gpu_id, epoch)
)
os.system(cmd)
