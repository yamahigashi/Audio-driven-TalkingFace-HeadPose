        ...
        if not self.opt.attention:
            ...
            if not self.opt.use_memory:
                ...
                if self.opt.fixindex != -1:
                    ...
                    top1_feature, top1_index = self.netmem.get_feature(
                        top1_idx, self.real.size()[0]
                    )
                    # print(top1_index, top1_idx)
                else:
                    query = self.netmem(self.resnet_input)
                    top1_feature, top1_index, top1_idx = self.netmem.topk_feature(
                        query, 1
                    )
            ...
            self.fake = (
                self.fake_B_mask * self.real[:, -self.opt.input_nc :]
