...
class MemorySeqModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument(
                "--test_use_gt", type=int, default=0, help="use gt feature in test"
            )
        parser.add_argument(
            "--attention",
            type=int,
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(
            ...
            opt.init_gain,
            self.gpu_ids,
            feat_dim=opt.iden_feat_dim,
        )

        self.netmem = Memory_Network(
            mem_size=opt.mem_size,
            color_feat_dim=opt.iden_feat_dim,
            spatial_feat_dim=opt.spatial_feat_dim,
            top_k=opt.top_k,
            alpha=opt.alpha,
        ...
        if (
            ...
            self.netD = networks.define_D(
                opt.input_nc * opt.Nw + opt.output_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
                opt.norm,
        ...
        if self.isTrain:
            ...
            self.optimizer_mem = torch.optim.Adam(
                self.netmem.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
            )
    ...
    def set_input(self, input):
        ...
        self.real_A = input["A" if AtoB else "B"].to(
            self.device
        )  # channel is input_nc * Nw
        self.real_B = input["B" if AtoB else "A"].to(
            self.device
        )  # channel is output_nc
    ...
    def forward(self):
        ...
        if self.opt.attention:
            if self.isTrain or self.opt.test_use_gt:
                self.fake_B_img, self.fake_B_mask = self.netG(
                    self.real_A, self.real_B_feat
                )
            else:
                query = self.netmem(self.resnet_input)
                # pdb.set_trace()
                top1_feature, top1_index, topk_index = self.netmem.topk_feature(
                    query, 1
                )
                top1_feature = top1_feature[:, 0, :]
                self.fake_B_img, self.fake_B_mask = self.netG(self.real_A, top1_feature)
            self.fake_B_mask = self._do_if_necessary_saturate_mask(
                self.fake_B_mask, saturate=self.opt.do_saturate_mask
            )
            self.fake_B = (
                self.fake_B_mask * self.real_A[:, -self.opt.input_nc :]
                + (1 - self.fake_B_mask) * self.fake_B_img
            )
    ...
    def backward_mem(self):
        ...
        self.loss_mem = self.netmem.unsupervised_loss(
            resnet_feature, self.real_B_feat, self.opt.iden_thres
        )
    ...
    def update_mem(self):
        with torch.no_grad():
            resnet_feature = self.netmem(self.resnet_input)
            replace = self.netmem.memory_update(
                resnet_feature, self.real_B_feat, self.opt.iden_thres, self.idx
            )
            if replace:
                self.replace += 1
            else:
    ...
    def backward_G(self):
        ...
        if self.opt.attention:
            ...
            self.loss_G_Att_smooth = (
                self._compute_loss_smooth(self.fake_B_mask)
                * self.opt.lambda_mask_smooth
            )
        # combine loss and calculate gradients
