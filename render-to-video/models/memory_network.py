...
class Memory_Network(nn.Module):
    def __init__(
        ...
        self.color_value = F.normalize(
            random_uniform((self.mem_size, self.color_feat_dim), 0, 0.01), p=1, dim=1
        ).to(self.device)

        self.spatial_key = F.normalize(
            random_uniform((self.mem_size, self.spatial_feat_dim), -0.01, 0.01), dim=1
        ).to(self.device)
    ...
    def unsupervised_loss(self, query, color_feat, color_thres):
        ...
        color_value_expand = torch.cat(
            [color_value_expand[:, :, idx] for idx in top_k_index], dim=0
        )

        color_feat_expand = torch.unsqueeze(color_feat, 2)
        color_feat_expand = torch.cat(
            [color_feat_expand for _ in range(self.top_k)], dim=2
        )

        # color_similarity = self.KL_divergence(color_value_expand, color_feat_expand, 1)
        color_similarity = torch.sum(
            torch.mul(color_value_expand, color_feat_expand), dim=1
        )
        ...
        neg_score, neg_index = torch.topk(
            torch.mul(top_k_score, 1 - loss_mask), 1, dim=1
        )
    ...
    def memory_update(self, query, color_feat, color_thres, top_index):
        ...
        self.spatial_key[case_index] = F.normalize(
            self.spatial_key[case_index] + query[memory_mask], dim=1
        )
        ...
        random_noise = random_uniform(
            (self.mem_size, 1), -self.age_noise, self.age_noise
        )[:, 0]
        ...
        return (
            torch.sum(memory_mask).cpu().numpy() == 1
        )  # for batch size 1, return number of replace
    ...
    def topk_feature(self, query, top_k=1):
        ...
        topk_feat = torch.cat(
            [
                torch.unsqueeze(self.color_value[topk_index[i], :], dim=0)
                for i in range(_bs)
            ],
            dim=0,
        )
        topk_idx = torch.cat(
            [torch.unsqueeze(self.top_index[topk_index[i]], dim=0) for i in range(_bs)],
            dim=0,
        )
    ...
    def get_feature(self, k, _bs):
        feat = torch.cat(
            [torch.unsqueeze(self.color_value[k, :], dim=0) for i in range(_bs)], dim=0
        )
