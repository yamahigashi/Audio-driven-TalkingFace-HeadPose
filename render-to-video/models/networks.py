...
class UnetACGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.model = UnetSkipConnectionACBlock(
            output_nc,
            ngf,
            input_nc=input_nc,
            submodule=unet_block,
...
class unet_generator_ac_adain(nn.Module):
    def __init__(self, input_nc, output_nc, ngf, feat_dim=512):
        ...
        self.mlp = MLP(
            feat_dim,
            self.get_num_adain_params(self.layers),
            self.get_num_adain_params(self.layers),
            3,
        )
...
class unet_decoder_ac_block(nn.Module):
    def __init__(
        ...
        self.deconv = nn.ConvTranspose2d(
            input_nc, int(input_nc / 2), ks, stride, padding
        )
        ...
        layers = []
        layers.append(
            nn.Conv2d(int(input_nc / 2), output_nc, kernel_size=7, stride=1, padding=3)
        )
        layers.append(nn.Tanh())
        self.img_reg = nn.Sequential(*layers)
        ...
        layers = []
        layers.append(
            nn.Conv2d(int(input_nc / 2), 1, kernel_size=7, stride=1, padding=3)
        )
        layers.append(nn.Sigmoid())
        self.attention_reg = nn.Sequential(*layers)
...
class UnetGeneratorRefine(nn.Module):
    ...
    def __init__(
        ...
        # construct unet structure
        unet_block = UnetSkipConnectionRefineBlock(
            ngf * 8,
            ngf * 8,
            input_nc=None,
            submodule=None,
        ...
        unet_block = UnetSkipConnectionRefineAddinputBlock(
            ngf * 2,
            ngf * 4,
            input_nc=None,
            dadd_nc=input_nc,
        ...
        self.model = UnetSkipConnectionRefineAddinputBlock(
            output_nc,
            ngf,
            input_nc=input_nc,
            dadd_nc=input_nc,
...
class UnetSkipConnectionRefineBlock(nn.Module):
    ...
    def __init__(
        ...
        upnorm = norm_layer(outer_nc)
        refine = nn.Conv2d(
            outer_nc, outer_nc, kernel_size=3, stride=1, padding=1, bias=use_bias
        )
...
class UnetSkipConnectionRefineAddinputBlock(nn.Module):
    ...
    def __init__(
        ...
        if outermost:
            upconv = nn.ConvTranspose2d(
                inner_nc * 2 + dadd_nc, outer_nc, kernel_size=4, stride=2, padding=1
            )
            down = [downconv]
            up = [uprelu, upconv, nn.Tanh()]
            model = down + [submodule] + up
