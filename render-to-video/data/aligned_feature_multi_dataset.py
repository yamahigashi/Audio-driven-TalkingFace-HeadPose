...
class AlignedFeatureMultiDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.isTrain or self.opt.test_use_gt:
            ...
            ss = AB_path2.split("/")
            if (
                self.opt.dataroot != "300vw_win3"
                and self.opt.dataroot != "lrwnewrender_win3"
                and ss[-3] != "19_news"
            ):
                B_feat = np.load(
                    os.path.join(self.opt.iden_feat_dir, ss[-2], ss[-1][:-4] + ".npy")
                )
            elif self.opt.dataroot == "300vw_win3":
                B_feat = np.load(
                    os.path.join(
                        self.opt.iden_feat_dir, ss[-3], ss[-2], ss[-1][:-4] + ".npy"
                    )
                )
            elif self.opt.dataroot == "lrwnewrender_win3":
                B_feat = np.load(
                    os.path.join(
                        self.opt.iden_feat_dir, ss[-4], ss[-3], ss[-2], "frame14.npy"
                    )
                )
            elif ss[-3] == "19_news":
                B_feat = np.load(
