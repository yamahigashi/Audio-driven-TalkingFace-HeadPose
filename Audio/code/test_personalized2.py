...
def dreassign2(video, audio, start, audiomodel="", num=300, debug=0, tran=0):
    ...
    if audiomodel != "":
        tardir = os.path.join(
            "../results/chosenbg",
            "%s_%s_%s" % (audio, video, audiomodel.replace("/", "_")),
        )
    ...
    for i in range(N):
        ...
        if tran == 0 or tran == 2:
            shutil.copy(
                os.path.join(pngdir, "frame%d.png" % (assigns[i] + start)),
                os.path.join(tardir2, "%05d.png" % i),
            )
        elif tran == 1:
            shutil.copy(
                os.path.join(pngdir, "frame%d_input2.png" % (assigns[i] + start)),
                os.path.join(tardir2, "%05d.png" % i),
            )

    if debug:
        os.system(
            "ffmpeg -loglevel panic -framerate 25 -i "
            + tardir2
            + "/%05d.png -c:v libx264 -y -vf format=yuv420p "
            + tardir2
            + ".mp4"
        )
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join("../audio/", audiobasen + ".wav")):
        ...
        print(
            "audio file not exists, please put in %s"
    ...
    if not os.path.exists(sample_dir + "/00000.npy"):
        add = (
            "--model_name ../model/%s/atcnet_lstm_%d.pth --pose 1 --relativeframe 0"
            % (audio_exp_name, audioepoch)
        )
        print(
            "python atcnet_test1.py --device_ids %d %s --sample_dir %s --in_file %s"
            % (gpu_id, add, sample_dir, in_file)
        )
        os.system(
            "python atcnet_test1.py --device_ids %d %s --sample_dir %s --in_file %s"
            % (gpu_id, add, sample_dir, in_file)
    ...
    num = 300
    bgdir = dreassign2(
        "19_news/" + person, audiobasen, start, audiomodel, num=num, tran=pingyi
    )
    ...
    relativeframe = 2
    os.system(
        "CUDA_VISIBLE_DEVICES=%d python render_for_view2.py %s %s %s %d %d %s"
        % (gpu_id, coeff_dir, coef_path1, save_dir, relativeframe, pingyi, tex2_path)
    )
    ...
    cmd = (
        "cd ../results; matlab -nojvm -nosplash -nodesktop -nodisplay -r \"alpha_blend_vbg('"
        + bgdir
        + "','"
        + srcdir
        + "'); quit;\""
    ...
    ## 5.gan
    sample_dir2 = "../../render-to-video/results/%s/test_%d/images%s/" % (
        ganmodel,
        ganepoch,
        seq,
    )
    ...
    os.system(
        "cd ../../render-to-video; python test_memory.py --dataroot %s --name %s --netG unetac_adain_256 --model test --Nw 3 --norm batch --dataset_mode single_multi --use_memory 1 --attention 1 --num_test 10000 --epoch %d --gpu_ids %d --imagefolder images%s"
        % (seq, ganmodel, ganepoch, gpu_id, seq)
    )
    ...
    os.system(
        ...
        + "/R_"
        + person
        + "_reassign2-00002_blend2_fake.png "
        + sample_dir2
        + "/R_"
        + person
        + "_reassign2-00000_blend2_fake.png"
    )
    os.system(
        "cp "
        + sample_dir2
        ...
        + "/R_"
        + person
        + "_reassign2-00001_blend2_fake.png"
    )

    video_name = os.path.join(
        sample_dir, "%s_%swav_results%s.mp4" % (person, audiobasen, post)
    )
    command = (
        "ffmpeg -loglevel panic -framerate 25  -i "
        + sample_dir2
        + "/R_"
        + person
        + "_reassign2-%05d_blend2_fake.png -c:v libx264 -y -vf format=yuv420p "
        + video_name
    )
    os.system(command)
    command = (
