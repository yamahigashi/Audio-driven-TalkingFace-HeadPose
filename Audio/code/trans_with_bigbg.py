...
def merge_with_bigbg(audiobasen, n):
    ...
    if not os.path.exists(os.path.join("../../Data", str(n), "transbig.npy")):
        cmd = "cd ../../Deep3DFaceReconstruction/; python demo_preprocess.py %d %d" % (
            n,
            n + 1,
        )
        os.system(cmd)
    ...
    for i in range(len(coeffs)):
        ...
        if seamlessclone == 0:
            ...
            img1 = Image.open(
                sample_dir2 + "/R_" + person + "_reassign2-%05d_blend2_fake.png" % i
            )
    ...
    transbigbgdir = os.path.join(sample_dir, "trans_bigbg")
    video_name = os.path.join(
        sample_dir, "%s_%swav_results_transbigbg.mp4" % (person, audiobasen)
    )
    command = (
        "ffmpeg -loglevel panic -framerate 25  -i "
        + transbigbgdir
