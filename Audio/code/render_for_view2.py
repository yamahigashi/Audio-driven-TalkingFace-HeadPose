...
class BFM:
    def __init__(self):
        ...
        self.keypoints = (
            np.squeeze(model["keypoints"]).astype(np.int32) - 1
        )  # 68 face landmark index, starts from 0
...
class RenderObject(object):
    ...
    def render(self, coef_path):
        ...
        result_output_path = os.path.join(
            "output/render_fuse", coef_path[13:-4] + "_render.png"
        )
    ...
    def render256(self, coef_path):
        ...
        result_output_path = os.path.join(
            "output/render_fuse", coef_path[13:-4] + "_render256.png"
        )
        if not os.path.exists(os.path.dirname(result_output_path)):
            os.makedirs(os.path.dirname(result_output_path))
    ...
    def render2(
        ...
        if coef2.shape[0] >= L + 8 and tran == 1:
            self.save_image2(
                final_images, result_output_path, tx=coef2[L + 6], ty=coef2[L + 7]
            )
        else:
            self.save_image(final_images, result_output_path)
    ...
    def render2_newtex(
        ...
        (
            ...
            face_color2,
            tri,
        ) = Reconstruction_for_render_new_given(coef1, self.facemodel, tex2_path)
        final_images = self.sess.run(
            self.rendered,
            feed_dict={
                self.faceshaper: face_shape_r.astype("float32"),
