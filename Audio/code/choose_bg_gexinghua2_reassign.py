...
def nearest2(sucai, query, sucai2, lastI, lamda=1.0 / 255.0, choice=1):
...
def nearest2IoU(query, sucai2, lastI, lamda=1.0 / 255.0):
...
def choose_bg_gexinghua2_reassign2(
    video, audio, start, audiomodel="", num=300, debug=0, tran=0, speed=2, aaxis=2
):
    ...
    for i in range(1, Ids.shape[0]):
        ...
        sucait = sucai[
            max(0, int(I - speed * period)) : min(num, I + int(speed * period)), :3
        ]
        ...
        if debug:
            if tran == 0:
                for j in range(Ids[i - 1], Ids[i] + 1):
                    shutil.copy(
                        os.path.join(pngdir, "frame%d.png" % (I + start)),
                        os.path.join(tempdir, "%05d.png" % j),
                    )
            else:
                for j in range(Ids[i - 1], Ids[i] + 1):
                    shutil.copy(
                        os.path.join(pngdir, "frame%d_input2.png" % (I + start)),
                        os.path.join(tempdir, "%05d.png" % j),
                    )
