...
def check_jacobians_are_nearly_equal(
    theoretical,
    numerical,
    ...
    """
    outlier_gradients = (
        np.abs(numerical - theoretical) / numerical > outlier_relative_error_threshold
    )
...
def expect_image_file_and_render_are_near(
    test_instance,
    sess,
    baseline_path,
    result_image,
    ...
    message = (
        "%s does not match. (%f of pixels are outliers, %f is allowed.). "
