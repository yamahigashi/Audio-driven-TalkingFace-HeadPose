...
class RenderTest(tf.test.TestCase):
    ...
    def runTriangleTest(self, w_vector, target_image_name):
        ...
        clip_init = clip_init * np.reshape(np.array(w_vector, dtype=np.float32), [3, 1])
        ...
        rendered_coordinates = tf.concat(
            [rendered_coordinates, tf.ones([self.image_height, self.image_width, 1])],
            axis=2,
        )
    ...
    def testRendersTwoCubesInBatch(self):
        ...
        look_at_1 = camera_utils.look_at(
            self.tf_float([[2.0, 3.0, 6.0]]), center, world_up
        )
        look_at_2 = camera_utils.look_at(
            self.tf_float([[-3.0, 1.0, 6.0]]), center, world_up
        )
        ...
        rendered = rasterize_triangles.rasterize(
            ...
            tf.stack([vertex_rgba, vertex_rgba]),
            self.cube_triangles,
            projection,
            self.image_width,
            self.image_height,
        ...
        with self.test_session() as sess:
            ...
            for i in (0, 1):
                ...
                baseline_image_path = os.path.join(
                    self.test_data_directory, baseline_image_name
                )
                test_utils.expect_image_file_and_render_are_near(
                    self, sess, baseline_image_path, image
                )
