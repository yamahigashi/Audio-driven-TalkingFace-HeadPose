...
class Trainer:
    def __init__(self, config):
        ...
        if config.lstm:
            if config.pose == 0:
                self.dataset = LRW_1D_lstm_3dmm(
                    config.dataset_dir, train=config.is_train, indexes=config.indexes
                )
            else:
                if config.dataset == "lrw":
                    self.dataset = LRW_1D_lstm_3dmm_pose(
                        config.dataset_dir,
                        train=config.is_train,
                        indexes=config.indexes,
                        relativeframe=config.relativeframe,
                    )
                    self.dataset2 = LRW_1D_lstm_3dmm_pose(
                        config.dataset_dir,
                        train="test",
                        indexes=config.indexes,
                        relativeframe=config.relativeframe,
                    ...
                    self.dataset = News_1D_lstm_3dmm_pose(
                        config.dataset_dir,
                        train=config.is_train,
                        indexes=config.indexes,
                        relativeframe=config.relativeframe,
                        newsname=config.newsname,
                        start=config.start,
                        trainN=config.trainN,
                        testN=config.testN,
                    )
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            for step, (coeff, audio, coeff2) in enumerate(self.data_loader):
                ...
                if config.less_constrain:
                    ...
                    ) + config.lambda_pose * self.mse_loss_fn(
                        fake_coeff[:, :, L:], coeff[:, :, L:]
                    )
                ...
                if (
                    ...
                    and (step) % (int(num_steps_per_epoch / 10)) == 0
                    and step != 0
                ) or (num_steps_per_epoch <= 100 and (step + 1) == num_steps_per_epoch):
                    if config.lstm:
                        for indx in range(3):
                            for jj in range(16):
                                name = "{}/real_{}_{}_{}.npy".format(
                                    config.sample_dir, cc, indx, jj
                                )
                                coeff2 = coeff.data.cpu().numpy()
                                np.save(name, coeff2[indx, jj])
                                if config.relativeframe:
                                    name = "{}/real2_{}_{}_{}.npy".format(
                                        config.sample_dir, cc, indx, jj
                                    )
                                    np.save(name, coeff2[indx, jj])
                                name = "{}/fake_{}_{}_{}.npy".format(
                                    config.sample_dir, cc, indx, jj
                                )
                        ...
                        vloss = 0
                        if config.dataset == "lrw":
                            for step, (coeff, audio, coeff2) in enumerate(
                        ...
                        torch.save(
                            ...
                            "{}/atcnet_lstm_{}.pth".format(config.model_dir, cc),
                        )
                    cc += 1
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="whether use relative frame value for pose",
    )
    ...
    parser.add_argument(
        "--model_name", type=str, default="../model/atcnet_pose0/atcnet_lstm_24.pth"
    )
    parser.add_argument("--preserve_mouth", type=bool, default=False)
    # for remove jittering
    parser.add_argument(
        "--smooth_loss", type=bool, default=False
    )  # smooth in time, similar to total variation
    parser.add_argument(
        "--smooth_loss2", type=bool, default=False
    )  # smooth in time, for expression
