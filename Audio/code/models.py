...
class GL_Discriminator(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(1024, 512), nn.ReLU(True), nn.Linear(512, 136), nn.Tanh()
        )
        ...
        self.lstm_fc = nn.Sequential(nn.Linear(256, 136), nn.Tanh())
    ...
    def forward(self, xs, example_landmark):
        hidden = (
            torch.autograd.Variable(
                torch.zeros(3, example_landmark.size(0), 256).cuda()
            ),
            torch.autograd.Variable(
                torch.zeros(3, example_landmark.size(0), 256).cuda()
            ),
...
class VG_net(nn.Module):
    def __init__(
        ...
        self.image_encoder2 = nn.Sequential(
            conv2d(128, 256, 3, 2, 1), conv2d(256, 512, 3, 2, 1)
        )

        self.landmark_encoder = nn.Sequential(nn.Linear(136, 64), nn.ReLU(True))
        ...
        self.lmark_feature = nn.Sequential(conv2d(256, 512, 3))
    ...
    def forward(self, image, landmarks, example_landmark):
        ...
        ex_landmark1 = self.landmark_encoder(
            example_landmark.view(example_landmark.size(0), -1)
        )
        ex_landmark1 = ex_landmark1.view(
            ex_landmark1.size(0), 1, image_feature.size(2), image_feature.size(3)
        )
        ...
        for step_t in range(landmarks.size(1)):
            ...
            landmark = landmark.view(
                landmark.size(0), 1, image_feature.size(2), image_feature.size(3)
            )
            ...
            inputs = self.bottle_neck(
                torch.cat([image_feature, landmark - ex_landmark], dim=1)
            )
        ...
        for step_t in range(landmarks.size(1)):
            ...
            v_feature1_f = (
                image_feature1 * (1 - lmark_atts[:, step_t, :, :, :])
                + v_feature1 * lmark_atts[:, step_t, :, :, :]
            )
        ...
        return (
            ...
            torch.stack(atts, dim=1),
            torch.stack(colors, dim=1),
            lmark_atts,
        )
