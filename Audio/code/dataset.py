...
class LRW_1D_lstm_landmark_pca(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.train == "train":
            lmark_path = os.path.join(
                self.lmark_root_path,
                self.train_data[index][0],
                self.train_data[index][1],
                self.train_data[index][2],
                self.train_data[index][2] + ".npy",
            )
        ...
        if self.train == "test":
            lmark_path = os.path.join(
                ...
                self.test_data[index][2],
                self.test_data[index][2] + ".npy",
            )
            mfcc_path = os.path.join(
                "../dataset/mfcc/",
                self.test_data[index][0],
...
class LRW_1D_lstm_3dmm(data.Dataset):
    def __init__(self, dataset_dir, train="train", indexes=range(80, 144)):
        self.train = train
        self.num_frames = 16
        if self.train == "train":
...
class LRW_1D_lstm_3dmm_pose(data.Dataset):
    def __init__(
        self, dataset_dir, train="train", indexes=range(80, 144), relativeframe=0
    ):
        self.train = train
        self.num_frames = 16
        if self.train == "train":
...
class News_1D_lstm_3dmm_pose(data.Dataset):
    def __init__(
        ...
        if self.train == "train":
            ...
            print(
                mfccs.shape
            )  # (24101, 28, 12) for Ben_Shapiro, (16282, 28, 12) for BBC_Carrie_Lam
            ...
            for i in range(self.trainN):
                coeff_path = os.path.join(
                    "../../Deep3DFaceReconstruction/output/coeff/",
                    self.newsname,
                    "frame%d.mat" % (self.start + i),
                )
                # print(coeff_path)
...
class LRW_1D_single_landmark_pca(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train == "train":
            ...
            r = random.choice([x for x in range(3, 25)])
            example_landmark = lmark[r, :]
            example_mfcc = mfcc[(r - 3) * 4 : (r + 4) * 4, 1:]

            while True:
                current_frame_id = random.choice([x for x in range(3, 25)])
...
class LRWdataset1D_single_gt(data.Dataset):
    def __init__(self, dataset_dir, output_shape=[128, 128], train="train"):
        self.train = train
    ...
    def __getitem__(self, index):
        ...
        if self.train == "train":
            ...
            landmark_path = (
                "../dataset/landmark1d/" + self.train_data[index][0][:-8] + ".npy"
            )
            ...
            r = random.choice([x for x in range(1, 30)])
            ...
            landmark_path = (
                "../dataset/landmark1d/" + self.test_data[index][0][:-8] + ".npy"
            )
            landmark = np.load(landmark_path) * 5.0
...
class LRWdataset1D_lstm_gt(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train == "train":
            ...
            right_img = torch.FloatTensor(
                16, 3, self.output_shape[0], self.output_shape[1]
            )
            for jj in range(16):
                this_frame = current_frame_id + jj
                image_path = (
                    "../dataset/regions/"
                    + self.train_data[index][0][:-7]
                    + "%03d.jpg" % this_frame
                )
                im = cv2.imread(image_path)
                im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
            ...
            right_landmark = landmark[
                self.train_data[index][1] - 1 : self.train_data[index][1] + 15
            ]
            ...
            right_landmark = landmark[
                self.test_data[index][1] - 1 : self.test_data[index][1] + 15
            ]
    ...
    def __getitem__(self, index):
        ...
        if self.train == "train":
            while True:
                ...
                tp = (np.dot(right_landmark.reshape(1, 6), EIGVECS))[0, :].reshape(
                    68, 3
                )
                tp = tp[:, :-1].reshape(-1)
                ...
                tp = (np.dot(example_landmark.reshape(1, 6), EIGVECS))[0, :].reshape(
                    68, 3
                )
                tp = tp[:, :-1].reshape(-1)
            ...
            while True:
                ...
                right_landmark = torch.FloatTensor(
                    (MS + np.dot(right_landmark.reshape(1, 6), EIGVECS)).reshape(-1)
                )
                ...
                example_landmark = torch.FloatTensor(
                    (MS + np.dot(example_landmark.reshape(1, 6), EIGVECS)).reshape(-1)
                )
    ...
    def __getitem__(self, index):
        ...
        if self.train == "train":
            ...
            image_path = os.path.join(
                "/mnt/ssd0/dat/lchen63/grid/data",
                self.train_data[index][0],
                self.train_data[index][0],
                "%05d.jpg" % (self.train_data[index][1] + 1),
            )

            landmark_path = os.path.join(
                "/mnt/ssd0/dat/lchen63/grid/data",
                self.train_data[index][0],
                self.train_data[index][0] + "_norm_lmarks.npy",
            )
            ...
            r = random.choice([x for x in range(1, 76)])
            ...
            image_path = os.path.join(
                ...
                self.test_data[index][0],
                self.test_data[index][0],
                "%05d.jpg" % (self.test_data[index][1] + 1),
            )

            landmark_path = os.path.join(
                "/mnt/ssd0/dat/lchen63/grid/data",
                self.test_data[index][0],
                self.test_data[index][0] + "_norm_lmarks.npy",
            )
            ...
            # load righ img
            image_path = (
                "/mnt/ssd0/dat/lchen63/lrw/demo/regions/" + self.demo_data[index][0]
            )
            landmark_path = (
                "/mnt/ssd0/dat/lchen63/lrw/demo/landmark1d/"
                + self.demo_data[index][1].replace("obama_", "obama_ge_")
            )
    ...
    def __getitem__(self, index):
        ...
        if self.train == "train":
            try:
                ...
                mfcc_path = os.path.join(
                    self.root_path,
                    self.train_data[index][0],
                    self.train_data[index][0] + "_mfcc.npy",
                )
        ...
        if self.train == "test":
            ...
            mfcc_path = os.path.join(
                self.root_path,
                self.test_data[index][0],
                self.test_data[index][0] + "_mfcc.npy",
            )
            ...
            r = random.choice([x for x in range(3, 70)])
            example_landmark = lmark[r, :]
            example_mfcc = mfcc[(r - 3) * 4 : (r + 4) * 4, 1:]
